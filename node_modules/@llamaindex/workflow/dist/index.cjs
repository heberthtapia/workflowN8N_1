...
var snapshot = require('@llamaindex/workflow-core/middleware/snapshot');
 ...
 */ const createEventEmitterTool = (name, event, workflowContext, description)=>{
    ...
    const toolDescriptionWithSchema = (description ?? event.schema.description ?? "Use this tool to send the event to the workflow.") + `\n\nPlease provide the event data in the following JSON schema: ${JSON.stringify(zodToJsonSchema.zodToJsonSchema(zod$1.z.object({
 ...
 * with multiple tools.
 */ class AgentWorkflow {
    constructor({ agents, rootAgent, memory, verbose }){
        ...
        this.executeToolCalls = async (event)=>{
            const { sendEvent } = workflowCore.getContext();
        ...
        this.processToolResults = async (event)=>{
            ...
            if (directResult) {
                ...
                if (isHandoff) {
                    ...
                    if (nextAgentName) {
                        ...
                        const messages = await this.stateful.getContext().state.memory.getLLM(this.agents.get(nextAgentName)?.llm);
                        if (this.verbose) {
            ...
            const messages = await this.stateful.getContext().state.memory.getLLM(this.agents.get(agent.name)?.llm);
            return agentInputEvent.with({
                input: messages,
                currentAgentName: agent.name
            });
 ...
 */ const agentHandler = (params)=>{
    return async (event)=>{
        const context = workflowCore.getContext();
...
Object.keys(snapshot).forEach(function (k) {
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
    enumerable: true,
    get: function () { return snapshot[k]; }
  });
});
