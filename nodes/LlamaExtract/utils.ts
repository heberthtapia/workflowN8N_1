...
async function runExtractionJob(
  ...
  const result: ExtractionJobResponse = await response.json() as ExtractionJobResponse;
  return result.id;
}
...
async function pollForJobCompletion(
  ...
  while (retries < maxRetries) {
    ...
    const status: JobStatusResponse = await response.json() as JobStatusResponse;
